...
var _ = Describe("Test templatization", Ordered, func() {
	...
	Describe("Policy with the ObjectNamespace variables", Ordered, func() {
		const (
			preReqs           = case13RsrcPath + "/case13_object_variables_prereqs.yaml"
			policyYAML        = case13RsrcPath + "/case13_object_variables.yaml"
			policyName        = "case13-object-variables"
			invalidPolicyYAML = case13RsrcPath + "/case13_invalid_ns.yaml"
	...
	FDescribe("Policy with the ObjectName variable", Ordered, func() {
		const (
		...
		It("Should omit ConfigMaps when $SKIP is returned", func(ctx SpecContext) {
			By("Creating the prerequisites")
			utils.Kubectl("apply", "-f", preReqs)

			By("Applying the " + skipPolicyName + " ConfigurationPolicy")
			utils.Kubectl("apply", "-n", testNamespace, "-f", skipPolicyYAML)

			By("By verifying that the ConfigurationPolicy is compliant and has the correct related objects")
			Eventually(func(g Gomega) {
				managedPlc := utils.GetWithTimeout(
					clientManagedDynamic, gvrConfigPolicy, skipPolicyName, testNamespace, true, defaultTimeoutSeconds,
				)
				...
				for idx := range relatedObjects {
					...
					relatedObject1NS, _, _ := unstructured.NestedString(relatedObject, "object", "metadata", "name")
					g.Expect(relatedObject1NS).To(
						Equal(fmt.Sprintf("%s%d", e2eBaseName, 2*idx+1)),
						"Related object name should match")
				}
			}, defaultTimeoutSeconds, 1).Should(Succeed())
			...
			for idx, cm := range configMaps.Items {
				labelAssertion := Expect(cm.ObjectMeta.Labels)
				var labelAssertionFn func(types.GomegaMatcher, ...interface{}) bool
				if idx == 1 {
					labelAssertionFn = labelAssertion.ToNot
				} else {
					labelAssertionFn = labelAssertion.To
				}
				labelAssertionFn(HaveKeyWithValue("case13", "passed"))
				labelAssertionFn(HaveKeyWithValue("name", cm.GetName()))
				labelAssertionFn(HaveKeyWithValue("namespace", cm.GetNamespace()))
			}
		})
